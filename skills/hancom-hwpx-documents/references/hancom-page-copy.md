...
If format-preserving text patching leaves empty paragraphs in edited table cells and the form overflows, inspect the changed cells first. Run `scripts/clean_empty_form_paragraphs.py` only with anchors that uniquely identify those changed cells and specify `--empty-position leading`, `trailing`, or `edges`; reserve `all` for cases where internal blank paragraphs are also proven residue. Use `--reset-text-extent` only when stale cached cell extents prevent automatic wrapping. The helper preserves the source namespace prefixes and ZIP entry properties. Always write a separate candidate, validate it, and reopen the candidate in Hancom before replacing a working file.
